...
class BookingControllerV2 extends Controller
{
    ...
    public function slotLockStatus(int $slotId, string $date): JsonResponse
    {
        if (! in_array(app()->environment(), ['local', 'testing'], true)) {
            return response()->json([
                'success' => false,
                'message' => 'Debug endpoint only available in local or testing environment.',
                'data' => null,
                'meta' => null,
            ], 403);
        }

        $service = app(SlotLockService::class);
        $locked = $service->isLocked($slotId, $date);

        return response()->json([
            'success' => true,
            'message' => 'Slot lock status retrieved.',
            'data' => [
                'slot_id' => $slotId,
                'date' => $date,
                'locked' => $locked,
